import { a as __toESM, t as __commonJSMin } from "./chunk-DSjvVL_1.mjs";
import { a as error, i as sortClasses, n as getTailwindConfig$1, o as warn, r as sortClassList } from "./sorter-BZkvDMjt.mjs";
import { a as expiringMap, n as maybeResolve, t as loadIfExists } from "./resolve-pWjAK-4f.mjs";
import { i as visit, n as cacheForDirs, r as spliceChangesIntoString } from "./utils-D8dQkKEd.mjs";
...
function transformCss(ast, env) {
	function tryParseAtRuleParams(name, params) {
		...
		try {
			return prettierParserCss.parsers.css.parse(`@import ${params};`, { ...env.options }).nodes[0].params;
		} catch (err) {
...
const { parsers, printers } = createPlugin([
	defineTransform({
		...
		load: [{
			...
			importer: () => import("prettier/plugins/html")
		}],
	...
	defineTransform({
		...
		load: [{
			...
			importer: () => import("prettier/plugins/glimmer")
		}],
		parsers: { glimmer: {} },
	...
	defineTransform({
		...
		parsers: {
			...
			meriyah: { load: [{
				...
				importer: () => import("prettier/plugins/meriyah")
			}] },
			acorn: { load: [{
				name: "prettier/plugins/acorn",
				importer: () => import("prettier/plugins/acorn")
			}] },
			flow: { load: [{
				name: "prettier/plugins/flow",
				importer: () => import("prettier/plugins/flow")
			}] },
			oxc: { load: [{
